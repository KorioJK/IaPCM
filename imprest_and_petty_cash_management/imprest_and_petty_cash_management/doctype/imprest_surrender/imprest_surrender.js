...
// frappe.ui.form.on("Imprest Surrender", {
// 	refresh(frm) {
...
frappe.ui.form.on("Imprest Surrender", {
    ...
    setup: function(frm) {
		...
		frm.set_query("imprest_or_petty_cash_entry", function() {
   ...
   imprest_or_petty_cash_entry: function(frm){
		frm.doc.imprest_entry = frm.doc.imprest_or_petty_cash_entry
		frm.refresh_field("imprest_entry")
   },
     ...
     if(frm.doc.imprest_entry){
         if(excess > 0 || remaining > 0){
             get_payment_mode_account(frm, frm.doc.payment_mode, function(account){             
                 ...
                 // creditRow.debit_in_account_currency = total;
                 creditRow.account =acc;
                
                if(excess > remaining){
                    creditRow.credit_in_account_currency = excess-remaining;
                }else{
                   creditRow.debit_in_account_currency = remaining-excess;
                }
   ...
   set_fields: function(frm){
    if (frm.doc.imprest_or_petty_cash_entry) {
         frappe.call({
             method: "frappe.client.get",
             args: {
                 doctype: "Imprest Requisition and Petty Cash",
                 name: frm.doc.imprest_or_petty_cash_entry //imprest_entry
             },
             callback: function(response) {
                 let linked_doc = response.message;
                 console.log(linked_doc);
                 // assuming field1 and field2 are fields you want to autofill
                 frm.set_value('company', linked_doc.company);
                 frm.set_value('employee', linked_doc.employee);
                 frm.set_value('posting_date', linked_doc.posting_date);
                 frm.set_value('payment_mode', linked_doc.payment_mode);
                 frm.set_value('expenditure',linked_doc.expenditure);
				 frm.set_value('narration',linked_doc.narration);
                }
            });
        }
    },
